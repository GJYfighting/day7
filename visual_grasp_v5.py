...
class VisualGrasp(Node):
    ...
    def geometry_targets(self, surface: list[float]) -> tuple[list[float], list[float], list[float]]:
        ...
        # The outer boxes span 59.8 mm along their long axis.  A 10 mm top-face
        # insertion places their usable side faces around the 50 mm block while
        # retaining 5 mm nominal table clearance.
    ...
    def analytic_gripper_clearance(self, q: list[float], visual: dict[str, object]) -> float:
        ...
        positions[str(self.geometry["gripper_driver_joint"])] = float(self.g["open_target"])
        minimum_z = math.inf
        for link, collision in ((self.geometry["left_finger_link"], self.geometry["left_finger_collision"]),
            ...
            transform = self.urdf.box_transform(self.c["base_frame"], box, positions)
            for sx in (-.5, .5):
                for sy in (-.5, .5):
                    for sz in (-.5, .5):
                        point = transform @ np.asarray([sx * box.size[0], sy * box.size[1], sz * box.size[2], 1.0])
