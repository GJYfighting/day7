...
class DomainRandomizer:
    ...
    def sample(self, seed, level, mode='fixed'):
        ...
        position = []
        for key, origin in zip(('base_x_m', 'base_y_m'), self.config['robot_spawn_world']):
            lo, hi = self.config['day4']['sampling'][key]
            center = (lo + hi) / 2
            width = (hi - lo) * span / 2
            position.append(float(origin + rng.uniform(center - width, center + width)))
        position.append(float(self.config['block_reset_world'][2]))
        ...
        return dict(seed=int(seed), level=level, enabled=enabled, position_world_m=position,
                    position_fraction=span, yaw_rad=math.radians(yaw),
                    ...
                    gain_scale=scale('gain_scale', 'gain_fraction'),
    ...
    def write_sdf(self, params):
        ...
        x, y, z = map(float, link.findtext('collision/geometry/box/size').split())
...
class DelayedPublisher:
    """Delay transport without blocking the ROS executor or freshness checks."""
    ...
    def publish(self, message):
        ...
        with self.condition:
            if self.error:
                raise TechnicalFailure('delayed publisher: '+str(self.error))
            if self.stopped:
                raise TechnicalFailure('delayed publisher already stopped')
            if not delay:
                self.publisher.publish(message)
            else:
                self.pending.append((time.monotonic()+delay, copy.deepcopy(message)))
...
class Day7GraspEnv:
    def __init__(self, config_path=ROOT / 'config.yaml'):
        ...
        self.nominal_gain = self.env.controller.base_gain
    ...
    def update_perception(self):
        ...
        try:
            ...
            value = json.dumps(self.params, allow_nan=False)
            request = SetParametersAtomically.Request()
            request.parameters = [Parameter('day7_domain', value=value).to_parameter_msg()]
    ...
    def reset(self, seed, level, mode):
        for publisher in (self.env.base_env.backend.arm_pub, self.env.base_env.backend.gripper_pub):
            publisher.clear()
        self.params = self.randomizer.sample(seed, level, mode)
        self.env.controller.base_gain = self.nominal_gain * self.params['gain_scale']
...
def run_episodes(mode, level, episodes, seed, *, resume=False, tag='manual'):
    ...
    runtime = None
    run_id = os.environ.get('DAY7_CHECK_RUN_ID', f'manual_{time.time_ns()}')
    records = []
    failures = 0
    valid = 0
    def alarm_handler(*_):
    ...
    try:
        while valid < episodes:
            ...
            print(f"DAY7_EPISODE={attempt} LEVEL={record['level_before']} VALID={valid}/{episodes} TECHNICAL={record['technical_failure']} SUCCESS={record['success']} REASON={record['technical_reason']}", flush=True)
...
def main():
    config = yaml.safe_load((ROOT / 'config.yaml').read_text())['day7']
